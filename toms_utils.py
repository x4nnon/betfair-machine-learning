    ...
    for day in os.listdir(month_folder):
        ...
        for event in os.listdir(day_dir):
            ...
            for file in os.listdir(event_dir):
                ...
                with open(archive_path, "rb") as source, open(
                    outfile_path, "wb"
                ) as dest:
...
def to_combined_csv(wins_folder, wins_combined_path):
    ...
    trading = betfairlightweight.APIClient(
        "username", "password", app_key="d5b6fltDUE03k4lQ"
    )
    # create listener
                ...
                for market_book in market_books:  # this is only one.
                    for runner in market_book.runners:
                        ...
                        for i in range(len(runner.ex.available_to_back)):
                            back_dict[
                                runner.ex.available_to_back[i]["price"]
                            ] = runner.ex.available_to_back[i]["size"]
                        for i in range(len(runner.ex.available_to_lay)):
                            lay_dict[
                                runner.ex.available_to_lay[i]["price"]
                            ] = runner.ex.available_to_lay[i]["size"]
                        for i in range(len(runner.ex.traded_volume)):
                            volume_dict[
                                runner.ex.traded_volume[i]["price"]
                            ] = runner.ex.traded_volume[i]["size"]

                        # Below will check traded volume at the last price traded
                        if temp_list[5] != "":
                            if (
                                temp_list[5] in volume_dict.keys()
                            ):  # need the extra for an edge case (doesn't seem like this could ever happen logically)
...
def create_individual_selection_csvs(combined_path):
    ...
    column_names = [
        ...
        "available_to_back_1_price",
        "available_to_back_1_size",
        "volume_traded_atBprice1",
        "available_to_back_2_price",
        "available_to_back_2_size",
        "volume_traded_atBprice2",
        "available_to_back_3_price",
        "available_to_back_3_size",
        ...
        "available_to_lay_1_price",
        "available_to_lay_1_size",
        "volume_traded_atLprice1",
        "available_to_lay_2_price",
        "available_to_lay_2_size",
        "volume_traded_atLprice2",
        "available_to_lay_3_price",
        "available_to_lay_3_size",
    ...
    for folder in os.listdir(combined_path):
        ...
        for selection_id in selection_ids:
            ...
            new_file_path = os.path.join(
                folder_path, str(selection_id) + ".csv"
            )  # horrible hacky method sorry.

            selection_df.columns = column_names
            selection_df = selection_df.drop(
                [
                    "index",
                    "available_to_back_3_price",
                    "available_to_back_3_size",
...
def check_bsp_and_remove_files(df_cc, combined_path):
    ...
    for folder in os.listdir(combined_path):
        ...
        for file in os.listdir(folder_path):
            if (file[1] != ".") & (
                file[0] != "."
            ):  # make sure it's not the combined or lock file
                selection_id = file[:-4]
                # print("-- ", selection_id)
                bsp_line = df_cc[
                    (df_cc["EVENT_ID"] == int(market_compare))
                    & (df_cc["SELECTION_ID"] == int(selection_id))
                ]
    ...
    print(
        "missing", missing
    )  # Note this will only show anything in the first run through


# because we remove the files if they aren't there.


def market_stream_to_analysis(
    market_id, market_stream_folder, min_selection_traded, folder_for_analysis
):
    """
    ...
    if market_id not in os.listdir(market_stream_folder):
        ...
        for selection_id in selection_ids:
            ...
            new_file_path = os.path.join(
                temp_path, str(selection_id) + ".csv"
            )  # horrible hacky method sorry.
...
def combined_path_selection_to_analysis(
    combined_path,
    csv_name,
    bsp_df,
    min_selection_traded=1000,
    max_price=30,
        ...
        for file in os.listdir(folder_dir):
            if file[-5] == "d":
                ...
                for i in range(len(time_brackets) - 1):
                    ...
                    # BSP
                bsp_value = bsp_df.loc[
                    (bsp_df["EVENT_ID"] == float(folder))
                    & (bsp_df["SELECTION_ID"] == int(file[:-4]))
                ]["BSP"].values[0]
    ...
    for specific in os.listdir(wins_folder):
                    ...
                    if seconds_to_start > 0:
                        if number_runners != market_book.number_of_active_runners:
                            if market_book.market_id not in market_nonrunner_drop_list:
                                print(
                                    "{} has a non_runner".format(market_book.market_id)
                                )
        ...
        for selection_id in selection_ids:
            ...
            if int(market_book.market_id[2:]) in combined_bsp["EVENT_ID"].values:
                new_df = combined_bsp[
                    combined_bsp["EVENT_ID"] == int(market_book.market_id[2:])
                ]
                if (
                    int(selection_id) in new_df["SELECTION_ID"].values
                ):  # then continue :)
                    selection_df = df_combined[
                        df_combined["SelectionId"] == selection_id
                    ]
                    ...
                    df = selection_df.loc[
                        selection_df["SelectionTotalMatched"] > min_selection_traded
                    ]
                    for i in range(len(time_brackets) - 1):
                        # print(all_prices_traded)
                        # filter all before a certain time to off feel free to explore this value.
                        ...
                        df_new2 = df_new.loc[df_new["SecondsToStart"] > time2]
                        df_WOM = df.loc[
                            (df["SecondsToStart"] > time)
                            & (df["SecondsToStart"] < time2)
                        ]
                        all_prices_traded = df_new["LastPriceTraded"].unique()
                        first = True  # to avoid nan
                        ...
                        # BSP
                    bsp_value = combined_bsp.loc[
                        (combined_bsp["EVENT_ID"] == int(market_book.market_id[2:]))
                        & (combined_bsp["SELECTION_ID"] == int(selection_id))
                    ]["BSP"].values[0]
                    analysis_dict["bsps"].append(bsp_value)
